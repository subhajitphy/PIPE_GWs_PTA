...
# coding: utf-8
"""
pred_phase_dnfs_realisation.py

DNFs SBI in REALISATION mode with *predicted phase encoding* (via PhaseProvider),
using the SAME data pipeline as your point-based EAUnifiedPE script:

  - load NPZ: X_B (P,R,L), Y_by_pulsar (P,R,nparam), phase_B (R,L) [phase_B unused here]
  - center each pulsar timeseries
  - add noise to ENTIRE dataset first (on flattened P*L per realisation) with SNR in [20,30]
  - deterministic split on realizations
  - standardize X and y using TRAIN stats only
  - DataLoader yields (xb, yb) only
  - PhaseProvider predicts phi internally from standardized xb (EAUnifiedPE will unstandardize via x_mean/x_std)

NOTES:
  - This script assumes you already have:
      * REALISATION/pkg/ea_model_hy_new.py  (EAUnifiedPE)
      * REALISATION/pkg/phase_pred.py       (PhaseProvider class with signature below)
      * POINT_REAL/pkgs/model_dnfs.py       (PosteriorNet class, flow implementation)
  - If your model_dnfs.PosteriorNet internally builds EAUnifiedPE, keep it.
    If it expects an EA passed in, see the small "ALT" block near model creation.
...
from ea_model_hy_new import EAUnifiedPE   # your updated EA model
from phase_pred import PhaseProvider      # your *new* PhaseProvider (phase_ckpt_path, base_len, device, ...)
import model_dnfs as mdl
from model_dnfs import PosteriorNet       # your DNFs posterior network
...
# Config (NPZ)
# ==========================================================
DATA_PATH  = "/scratch/projects/CFP03/CFP03-CF-051/projects/gen_data/new_approach/con/fix/"
...
CSV_PATH  = os.path.join(SAVE_DIR, "loss_log_dnfs_pred_phase.csv")

# ==========================================================
# Helpers (match your point-inference pipeline)
# ==========================================================
...
def _unpack_batch(batch):
    """
    Supports your two-mode loader:
      - (xb, yb)
      - (xb, phib, yb)   [only if you ever enable true-phase tensors]
    """
...
phase_B  = data["phase_B"]       # (R,L) [unused here unless USE_TRUE_PHASE]
...
THETA_DIM = len(tidx)
mdl.THETA_DIM = THETA_DIM  # keep flow theta dim consistent
...
# Only if you ever want to feed true phase tensors (NOT used in pred-phase mode)
if USE_TRUE_PHASE:
...
# ==========================================================
# SECTION 7 — DataLoaders (TWO-MODE yield)
# ==========================================================
pin = (device.type == "cuda")
loader_gen = torch.Generator().manual_seed(SEED)

if USE_TRUE_PHASE:
    train_ds = TensorDataset(X_tr_t, phi_tr_t, y_tr_t)  # (xb, phib, yb)
    val_ds   = TensorDataset(X_va_t, phi_va_t, y_va_t)
else:
    train_ds = TensorDataset(X_tr_t, y_tr_t)            # (xb, yb)
...
# ==========================================================
# SECTION 8 — PhaseProvider (predicted φ) for EA phase encoding
# ==========================================================
phase_provider = None
...
# ==========================================================
# SECTION 9 — Build DNFs posterior model
# ==========================================================
# Most likely your model_dnfs.PosteriorNet internally builds an EAUnifiedPE context net
# and will call phase_provider if use_phase=True and phib is None.
model = PosteriorNet(
    seq_len=L,
    use_phase=USE_PHASE,                     # <- True (because we want pred-phase encoding)
    phase_provider=phase_provider,           # <- predicted phase provider
    x_mean=X_mean,
    x_std=X_std,
).to(device)

# ---- ALT (only if your PosteriorNet supports passing an EA explicitly) ----
# If you have a PosteriorNet that expects an EA model instance, do:
# ea = EAUnifiedPE(
#     seq_len=L,
#     out_dim=THETA_DIM,    # not used by flow, but required by ctor
#     patch=20, heads=8, d_model=128, depth=4, dim_ff=256, mem_size=40,
#     weighting="learned",
#     alpha_pos=1.0, alpha_phase=1.0, alpha_snr=1.0,
#     use_posenc=True,
#     use_phasepe=True,     # IMPORTANT: phase encoding ON
#     use_snrenc=False,
#     cnn_stem=False,
...
#     x_std=X_std,
# ).to(device)
# model = PosteriorNet(ea=ea, use_phase=USE_PHASE).to(device)
...
def per_target_z2(model, loader):
    ...
    Your older script did:
      h = model.cond(xb, phib)
      z, _ = model.flow.fwd_to_z(yb, h)
    ...
    for batch in loader:
        xb, phib, yb = prepare_batch(batch, device)
        h = model.cond(xb, phib if model.use_phase else None)
        z, _ = model.flow.fwd_to_z(yb, h)
        acc += (z**2).mean(dim=0).detach().cpu()
        nb += 1
    return (acc / max(nb, 1))

# ==========================================================
# LIVE PLOTS (same style as older script)
# ==========================================================
